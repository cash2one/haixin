var t = 10;
var sendCode = true;
var ahost = "http://a.jiemian.com";
//var _jm_ppt_id = $.cookie('_jm_ppt_id');
var addCommentUrl = ahost+"/index.php?m=comment&a=apiAdd";
var addCommentReplyUrl = ahost+"/index.php?m=comment&a=apiAddReply";
var delCommentUrl = ahost+"/index.php?m=comment&a=delCommentAjax";
var addPraiseUrl = ahost+"/index.php?m=comment&a=addPraiseDo";
var addCollectUrl = ahost+"/index.php?m=Collect&a=addCollect";
var getCommentReplyUrl = ahost+"/index.php?m=comment&a=getCommentReply";
var industryUrl = ahost+"/index.php?m=comment&a=industryDo";
var checkLoginUrl = ahost+"/index.php?m=comment&a=newIsLogin";
var getCommentUrl = ahost+"/index.php?m=comment&a=getlistComment";
//jsonp
...
function addComment(id) {
    ...
    if (t == 10) {
        var url = addCommentUrl + "&aid=" + id +"&content=" + encodeURIComponent(content);
    ...
    $.getJSON(url+"&callback=?",function(data){
        if(data.code == 0){
            ...
            $('#zhan').css({'margin-top': '-10px', 'display': 'block', 'position': 'absolute', 'left': '0'})
                ...
                .animate({"margin-top": "-55px"}, 'slow', function () {
                    $(this).fadeIn('fast').remove();
                })
            $("#praise" + id).html("(" + data.praise + ")");
        }else if(data.code == 2){
            $('#zhan').remove();
            $("#jia-"+id).append('<div id=\"zhan\"><b>-1<\/b><\/div>');
            $('#zhan').css({'margin-top':'-10px','display':'block','position':'absolute','left':'0'})
...
function addCommentReply(id, aid, comment_id) {
    ...
    var url = addCommentReplyUrl + "&reply_id=" + id +"&aid=" + aid + "&reply_comment=" + encodeURIComponent(reply_comment) + "&default_type=2";
    $.getJSON(url+"&callback=?",function(data){
        if(data.code == 0){
            if (id == comment_id) {//如果是回复的评论就放到最前面
...
function cai() {
    ...
    if (typeof(_jm_ppt_id) == "null" || _jm_ppt_id == '' || _jm_ppt_id == null) {
        ...
        $.ajax({
            type: "GET",
            url: ahost+'/index.php?m=dingcai&a=cai',
            data: {aid: aid},
            success: function (result) {
                var obj = eval(result);
                var rs = obj.data;
...
//    $.ajax({
//        type: "GET",
//        url: ahost+'/index.php?m=article&a=getArticle',
//        data: {aid: aid},
//        success: function (result) {
//            var obj = eval(result);
//            var rs = obj.data;
